...
function getParamsFromOptions(options) {
  ...
  if (options.forwardedFor) {
    params._requestCustomizer = function (req) {
      req.set('authok-forwarded-for', options.forwardedFor);
    };
  }
  if (options.type) {
...
class OAuthAuthenticator {
   ...
   * @param  {string}              options.baseUrl                     The Authok account URL.
   ...
   *
   *   <a href="https://authok.com/docs/api/authentication#database-ad-ldap-active-">
   *     API Docs
   *   </a>.
   * </caption>
   *
   ...
   * };
   *
   * authok.oauth.signIn(data, function (err, userData) {
   *   if (err) {
   *     // Handle error.
   *   }
   *
   ...
   * };
   *
   * authok.oauth.passwordGrant(data, function (err, userData) {
   *   if (err) {
   *     // Handle error.
   *   }
   *
  ...
  passwordGrant(userData, options, cb) {
    ...
    if (typeof data.realm === 'string' && data.realm.split().length !== 0) {
      data.grant_type = 'http://authok.com/oauth/grant-type/password-realm';
    }
   ...
   * };
   *
   * authok.oauth.refreshToken(data, function (err, data) {
   *   if (err) {
   *     // Handle error.
   *   }
   *
   ...
   *   from successful authentication, exchange the code for authok
   *   credentials. It will return JSON with the access_token and id_token.
   *   More information in the
   *   <a href="https://authok.com/docs/api/authentication#authorization-code-grant">
   *     API Docs
   *   </a>.
   * </caption>
   *
   ...
   * };
   *
   * authok.oauth.authorizationCodeGrant(options, function (err, userData) {
   *   if (err) {
   *     // Handle error.
   *   }
   *
   ...
   * @param   {string}    options.redirect_uri     The URL to which Authok will redirect the browser after authorization has been granted by the user.
   * @param {Function} cb Callback
   * @returns  {Promise|undefined}
   */
