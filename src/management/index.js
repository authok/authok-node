 ...
 * Management API SDK.
 *
 * The Authok Management API is meant to be used by back-end servers or trusted
 * parties performing administrative tasks. Generally speaking, anything that
 * can be done through the Authok dashboard (and more) can also be done through
 * this API.
 *
 * @example <caption>
 *   Initialize your client class with an API v2 token (you can generate one
 *   <a href="https://authok.com/docs/apiv2">here</a>) and a domain.
 * </caption>
 *
...
class ManagementClient {
   ...
   *
   * @example
   * authok.deleteRule({ id: RULE_ID }, function (err) {
   *   if (err) {
   *     // Handle error.
   *   }
   *
   ...
   * };
   *
   * authok.getUsers(params, function (err, users) {
   *   console.log(users.length);
   * });
   * @param   {object}    [params]               Users params.
   * @param   {number}    [params.search_engine] The version of the search engine to use.
   * @param   {string}    [params.q]             User Search string to filter which users are returned. Follows Lucene query string syntax as documented at https://docs.authok.cn/api/management/v1#!/Users/get_users.
   * @param   {number}    [params.per_page]      Number of results per page.
   * @param   {number}    [params.page]          Page number, zero indexed.
   * @param   {Function}  [cb]                   Callback function.
   * @returns  {Promise|undefined}
   ...
   * </caption>
   *
   * authok.getUsersByEmail(email, function (err, users) {
   *   console.log(users);
   * });
   ...
   * };
   *
   * authok.createPasswordChangeTicket(params, function (err) {
   *   if (err) {
   *     // Handle error.
   *   }
   * });
   ...
   * };
   *
   * authok.createEmailVerificationTicket(data, function (err) {
   *   if (err) {
   *     // Handle error.
   *   }
   * });
   ...
   *
   * @example
   * authok.deleteHook({ id: HOOK_ID }, function (err) {
   *   if (err) {
   *     // Handle error.
   *   }
   *
   ...
   * authok.removeHookSecrets(params, data, function (err) {
   *   if (err) {
   *     // Handle error.
   *   }
   *
