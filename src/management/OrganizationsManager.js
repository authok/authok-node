 ...
 * on Authok OrganizationsManager.
 */
class OrganizationsManager {
  ...
  constructor(options) {
     ...
     * {@link https://docs.authok.cn/api/v1}.
     *
    ...
    this.organizations = new RetryRestClient(authokRestClient, options.retry);

    const connectionsInRoleClient = new AuthokRestClient(
    ...
    const membersClient = new AuthokRestClient(
    ...
    const invitationClient = new AuthokRestClient(
    ...
    const rolesClient = new AuthokRestClient(
    ...
    const organizationByNameClient = new AuthokRestClient(
   ...
   * @param   {string}  data.client_id                Authok client used to resolve the default application login URI. This endpoint must expect &invitation=... and &organization=... parameters (added by API2) to continue the flow with /authorize. If client_id  does not have configured login URI, use the tenant level default login route if configured, otherwise return 400
