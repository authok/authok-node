...
class PromptsManager {
  ...
  constructor(options) {
     ...
     * {@link https://docs.authok.cn/api/management/v1#!/Prompts Prompts endpoint}.
     *
     ...
     * {@link https://docs.authok.cn/api/management/v1#!/Prompts/get_custom_text_by_language Custom Text endpoint}
     *
     *
     * @type {external:RestClient}
     */
    const customTextByLanguageauthokRestClient = new AuthokRestClient(
      `${options.baseUrl}/prompts/:prompt/custom-text/:language`,
      clientOptions,
      options.tokenProvider
    );
    this.customTextByLanguage = new RetryRestClient(
      customTextByLanguageauthokRestClient,
      options.retry
    );
  }
