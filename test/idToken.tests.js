...
const expectations = {
  ...
  issuer: 'https://tokens-test.authok.com/',
  nonce: 'a1b2c3d4e5',
  clock: Date.now(),
};
...
describe('idToken.validate', () => {
  ...
  it('should throw when the expected issuer is not in the claim', () => {
    expect(() => {
      ...
      'Issuer (iss) claim mismatch in the ID token; expected "ExpectedIssuer", found "https://tokens-test.authok.com/"'
    );
  });
