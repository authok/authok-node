...
describe('AuthenticationClient', () => {
  ...
  describe('client info', () => {
    it('should configure instances with default telemetry header', () => {
      ...
      const client = new AuthenticationClientProxy({ token: 'token', domain: 'authok.com' });

      const requestHeaders = {
        'Authok-Client': 'eyJuYW1lIjoidGVzdC1zZGsiLCJ2ZXJzaW9uIjoidmVyLTEyMyJ9',
    ...
    it('should configure instances without telemetry when "name" property is empty', () => {
      ...
      expect(client.oauth.oauth.options.headers).to.not.have.property('Authok-Client');
      expect(client.database.dbConnections.options.headers).to.not.have.property('Authok-Client');
      expect(client.passwordless.passwordless.options.headers).to.not.have.property('Authok-Client');
      expect(client.users.headers).to.not.have.property('Authok-Client');
      expect(client.tokens.headers).to.not.have.property('Authok-Client');
    });
  ...
  describe(`verifySMSCode`, () => {
    before(function () {
      this.client = new AuthenticationClient({ token: 'token', domain: 'authok.com' });
