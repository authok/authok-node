...
describe('OAuthAuthenticator', () => {
  ...
  describe('#signIn', () => {
    ...
    it('should include the Authok client ID in the request', function (done) {
      nock.cleanAll();
    ...
    it('should make it possible to pass authok-forwarded-for header', function (done) {
      nock.cleanAll();

      const request = nock(API_URL)
        .post(path, function () {
          return this.getHeader('authok-forwarded-for') === options.forwardedFor;
        })
        .reply(200);
  ...
  describe('#passwordGrant', () => {
    ...
    it('should require a password', function () {
      const auth = this.authenticator;
      const signIn = auth.passwordGrant.bind(auth, { username: 'samples@authok.com' });
    ...
    it('should include the Authok client secret in the request', function (done) {
      nock.cleanAll();
      ...
      const request = nock(API_URL)
        .post(
          ...
          (body) =>
            ...
            body.grant_type === 'http://authok.com/oauth/grant-type/password-realm'
        )
        .reply(200);
  ...
  describe('#clientCredentials', () => {
    ...
    it('should include the Authok client ID and secret in the request', function () {
      nock.cleanAll();
