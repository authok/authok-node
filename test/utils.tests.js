...
describe('Utils', () => {
  ...
  describe('maybe decode', () => {
    it('encodes regular user_id', () => {
      const normalId = 'authok|1234';
      expect(utils.maybeDecode(normalId)).to.be.equal('authok%7C1234');
    });
    ...
    it('does not encode an already encoded user_id', () => {
      const maliciousId = 'authok%7C1234';
      expect(utils.maybeDecode(maliciousId)).to.be.equal('authok%7C1234');
    });
  });
});
