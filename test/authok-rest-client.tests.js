...
const AuthokRestClient = require('../src/AuthokRestClient');

const API_URL = 'https://tenant.authok.com';

describe('AuthokRestClient', () => {
  before(function () {
    this.providerMock = {
      getAccessToken() {
  ...
  it('should raise an error when no options is provided', () => {
    expect(() => {
      new AuthokRestClient('/some-resource');
    }).to.throw(ArgumentError, 'Must provide options');
  });

  describe('`get`', () => {
    it('should encode params.id on `get` requests', function (done) {
      nock(API_URL).get('/some-resource/authok%7C1234').reply(200, { data: 'value' });

      const options = {
        headers: {},
      };
      const client = new AuthokRestClient(
        `${API_URL}/some-resource/:id`,
        options,
        this.providerMock
      );
      client.get({ id: 'authok|1234' }, (err, data) => {
    ...
    it('should not encode other params on `get` requests', function (done) {
      nock(API_URL)
        .get('/some-resource/1234?otherEncoded=authok%7C6789&other=foobar')
      ...
      client.get({ id: '1234', other: 'foobar', otherEncoded: 'authok|6789' }, (err, data) => {
    ...
    it('should accept only a callback', function (done) {
      ...
      const client = new AuthokRestClient(`${API_URL}/some-resource`, options, this.providerMock);
      client.get({}, (err, data) => {
        expect(data).to.deep.equal({ data: 'value' });
        done();
        nock.cleanAll();
  ...
  describe('`patch`', () => {
    it('should encode params.id on `patch` requests', function (done) {
      nock(API_URL).patch('/some-resource/authok%7C1234%2F5678').reply(200);
      ...
      client.patch({ id: 'authok|1234/5678' }, { data: 'udpate ' }, (err) => {
        expect(err).to.be.null;
        done();
        nock.cleanAll();
      });
  ...
  describe('`update`', () => {
    it('should encode params.id on `update` requests', function (done) {
      nock(API_URL).put('/some-resource/authok%7C1234%2F5678').reply(200);
      ...
      client.update({ id: 'authok|1234/5678' }, { data: 'udpate ' }, (err) => {
        expect(err).to.be.null;
        done();
        nock.cleanAll();
      });
  ...
  describe('`delete`', () => {
    it('should encode params.id on `delete` requests', function (done) {
      nock(API_URL).delete('/some-resource/authok%7C1234%2F5678').reply(200);
      ...
      client.delete({ id: 'authok|1234/5678' }, (err) => {
        expect(err).to.be.null;
        done();
        nock.cleanAll();
      });
  ...
  it('should accept a callback and handle errors', (done) => {
    ...
    const client = new AuthokRestClient(`${API_URL}/some-resource`, options, providerMock);
    client.getAll((err) => {
  ...
  it('should catch error when provider.getAccessToken throws an error', (done) => {
    ...
    const client = new AuthokRestClient('/some-resource', {}, providerMock);
    client.getAll().catch((err) => {
      expect(err).to.not.null;
      expect(err.message).to.be.equal('Some Error');
      done();
